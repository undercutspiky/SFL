...
def _make_masks(slide_path, xml_path, mask_level, make_map, **args):
    ...
    # Draw tumor mask
    # Fill up tumors first, then draw normal boundaries and fill those up with 0
...
def _record_patches(center_size,
    ...
    for i in range(margin, width - margin):
        for j in range(margin, height - margin):
            ...
            # Coords correspond to the centre only, not the entire patch
...
def generate_file(patient, node, xml_path, slide_path, folder_path):
    ...
    df['patient'] = patient
    df['node'] = node
    # TODO: comment this out if you don't want to generate PNGs with visualisations
    _write_masks(**args)
...
def generate_files(slide_root, output_root):
    ...
    # TODO: Modify the path. It should point to the directory containing XML annotations.
        ...
        for file in files:
            if file.endswith('.xml') and not file.startswith('._'):
                ...
                try:
                    ...
                    center_dir_name = f'center_{int(prefix.split("_")[1]) // 20}'
                    df = generate_file(
                        patient=prefix.split('_')[1],
                        node=prefix.split('_')[3],
                        xml_path=os.path.join(root, file),
                        slide_path=os.path.join(slide_root, 'training', center_dir_name, f'{prefix}.tif'),
                        folder_path=os.path.join(slide_root, 'masks', prefix))
...
if __name__ == '__main__':
    ...
    generate_files(slide_root=_args.slide_root, output_root=_args.output_root)
