...
from preprocess.BCSS.generate_all_patch_coords import CENTER_SIZE


def write_patch_images_from_df(slide_root, output_root):
    df = pd.read_csv(os.path.join(slide_root, 'metadata.csv'), index_col=0, dtype={'file_name': 'str'})
    ...
    for name, group in df.groupby('file_name'):
        ...
        slide = cv2.imread(os.path.join(slide_root, 'images', name))
